...
/**
 * class of Djkstra Graph Placehholder for Backend
 */
public class DijkstraGraphBD<T> implements IDijkstraGraph<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        if (vertices.get(start) == null || vertices.get(end) == null) {
            throw new NoSuchElementException("The path from " + start + " to " + end + " does not exist");
        }

        // get the start edge
        Vertex startEdge = vertices.get(start);
        // frontiers (all new path to go)
        PriorityQueue<Path> frontier = new PriorityQueue<>();
        // all existing path found
        Hashtable<Vertex, Path> visited = new Hashtable<>();

        frontier.add(new Path(startEdge));
        ...
        while (!frontier.isEmpty()) {
            ...
            if (!visited.containsKey(currentPath.end))
                visited.put(currentPath.end, currentPath);

            // add all edge, convert them to a path to them
            for (Edge newEdge : currentPath.end.edgesLeaving) {
                Vertex newEnd = newEdge.target;
                Path newPath = new Path(currentPath, newEdge);
                // if travelled
                if (visited.containsKey(newEnd)) {
                    // if (paths.get(newEnd).compareTo(newPath) > 0) {
                    // paths.replace(newEnd, newPath);
        ...
        // no path found when loop the whole graph
        throw new NoSuchElementException("The path from " + start + " to " + end + " does not exist");
    }
    ...
    public boolean containsEdge(T source, T target, int weight) {
        return containsEdge(source, target);
    }

    @Override
    public List<T> allNeighborVertices(T source) {
        List<Edge> neighbors = vertices.get(source).edgesLeaving;
        List<T> data = new ArrayList<>();
        for (Edge t : neighbors) {
            data.add(t.target.data);
        }
        return data;
    }
    ...
    public List<T> getAllVertices() {
        return (List<T>) Arrays.asList(vertices.keySet().toArray());
    }
